...
def write_log_gps(read_lon, read_lat, read_time):
    with open("log_gps.txt", "a") as f:
        #f.write(f"{read_lon},{read_lat},{time}\n")
        f.write(f"{read_lon},{read_lat},{read_time}\n")
...
def get_lon_lat_time():
	...
	while (read_lon == "" or read_lat == "") and attempt < 10:
		attempt += 1
		if attempt == 1:
...
while True:
	...
	if read_lon != "" or read_lat != "":
		write_log_gps(read_lon, read_lat, read_time)
