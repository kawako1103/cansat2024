...
def phase3(goal_pos, port, baudrate, reset_pin, file_path_lora,file_path_al):
    ...
    while not (rt.isGoal() and rt.longitude_flag==True and rt.latitude_flag==True) :
        ...
        while abs(deg) > 15.0 and times < 40:
            ...
            with open(file_path_al, mode='a', newline='') as file:
                ...
                writer.writerow(["-----------------Turning phase ---------------"])
                writer.writerow([f"before turn|| GPS:{gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}"])
                writer.writerow([f"before turn|| AngleDiff is {deg}"])

            print(f"before turn|| GPS:{gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}")
            ...
            if deg > 0:
                ...
                with open(file_path_al, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([f"Turn right! status:{status}"])
                ...
                with open(file_path_al, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([f"Turn left! status:{status}"])
            ...
            print(f"after turn|| GPS:{gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}")
            print(f"after turn|| goal_flag:{rt.goal_flag}, longitude_flag:{rt.longitude_flag}, latitude_flag:{rt.latitude_flag}")
            
            ## log
            with open(file_path_al, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([f"after turn|| GPS:{gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}"])
        ...
        print(f"before going forward||  GPS:{gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}") 
        ...
        with open(file_path_al, mode='a', newline='') as file:
            ...
            writer.writerow(["-----------------Forward movement phase ---------------"])
            writer.writerow([f"befor move forward|| {gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}"])
            writer.writerow([f"Move forward!"])
        ...
        with open(file_path_al, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([f"after move forward|| {gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}"])

        print(f"after going forward||  GPS:{gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}") 
        robot.stop() 
        rt.stop()
    ...
    with open(file_path_al, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([f"Arrival point|| GPS:{gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}"])
