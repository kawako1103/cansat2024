...
def phase3(goal_pos, port, baudrate, reset_pin, file_path_lora,file_path_al):
    ...
    while not (rt.isGoal() and rt.longitude_flag==True and rt.latitude_flag==True) :
        ...
        while abs(deg) > 15.0 and times < 40:
            ...
            print(f"after turnning: {gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}, Velocity: {velocity}")
        ...
        print(f"End Turn-phase ! AngleDiff is {deg} (>15.0)")
        status = robot.move(0.75, 3)
        print("Move forward! status:{status}")
        print(f"after going forward: {gps_pos}, Azimuth: {azimuth}, AngleDiff: {deg}, Distance: {distance}, Velocity: {velocity}") 
        robot.stop()
