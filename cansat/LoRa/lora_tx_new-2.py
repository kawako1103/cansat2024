        ...
        with open(self.file_path, "r") as file:
            ...
            for i in range(self.last_index, len(lines)):
                ...
                if len(parts) >= 2:
                    ...
                    deg=parts[4]
                    distance=parts[5]
                    velocity_value=parts[6]
                    status=parts[7]
                    vec_latlon.append((time_stamp,latitude, longitude,azimuth, deg, distance,velocity_value,status ))

        self.last_index = len(lines)
        print(f"self.last_index:{self.last_index}")
        print(f"vec_latlon:{vec_latlon}")
        return vec_latlon
        ...
        with open(self.log_file_path, 'a') as log_file:
            if not vec_latlon:
                ...
                for lat_lon in vec_latlon:
                    time_stamp,latitude, longitude,azimuth, deg, distance,velocity_value,status=lat_lon
                    # last_lat, last_lon = lat_lon
                    # output = f"4321FFFF{last_lat},{last_lon}\r\n"
                    output = f"4321FFFF{time_stamp},{latitude},{longitude},{azimuth},{deg},{distance},{velocity_value},{status}\r\n"
                    self.ser.write(output.encode('ascii'))
                    log_file.write(output)
                    print(output.encode('ascii'))
                    time.sleep(1)
...
def lora_tx_release_pre2():
    ...
    FILE_PATH = "/home/cansat-stu/cansat/test_log_mayu3.csv"
